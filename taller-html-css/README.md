...
## Cómo ejecutar
1. Clonar este repositorio o descargar el archivo `.zip` del proyecto.
...
1. **Uso de Flexbox para la sección de tarjetas (`.cards`):** Se implementó `display: flex` con `flex-wrap: wrap` y `flex: 1 1 280px` para lograr que los servicios se reorganicen solos según el tamaño de la pantalla sin romper la estructura.
2. **Sistema de Variables en CSS (`:root`):** Se definieron colores, espaciados y radios de borde globales (`--color-primary`, `--space`, `--border-radius`) para asegurar coherencia visual y facilitar cambios de diseño futuros en un solo lugar.
3. **Accesibilidad e interacción en el formulario:** Se asociaron estrictamente las etiquetas `<label>` con sus respectivos `<input>` mediante los atributos `for`/`id`, y se configuró `:focus-visible` con el color de acento para usuarios que navegan mediante el teclado.
